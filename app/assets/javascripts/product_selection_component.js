...
function connect() {
  ...
  if (firstVariant) {
    // Remove `setTimeout` needed to select first variant
    // once a fix is released with the new version of StimulusJS
...
function updateOptions(nextTypeName, optionIndex) {
  ...
  allNextOptions.forEach((radio) => {
    if (!nextOptionValues.includes(parseInt(radio.value, 10))) {
      ...
      radio.disabled = false; // eslint-disable-line no-param-reassign
...
function renderMasterView() {
  const { masterStock } = document.querySelector("[data-master-stock]").dataset;
  this.updateStockView(JSON.parse(masterStock));
}
